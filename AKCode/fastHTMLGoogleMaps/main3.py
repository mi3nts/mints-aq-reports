...
def init_map_js():
    return NotStr("""
        function initMap() {
            ...
            // Add pollution layer using geemap
            const pollutionLayer = new google.maps.ImageMapType({
                getTileUrl: function(coord, zoom) {
                    const eeMapOptions = geemap.ee.Image('COPERNICUS/S5P/NRTI/L3_NO2')
                    ...
                    const mapId = ee.data.getMapId(eeMapOptions);
                    return mapId.tile_fetcher.url_format.replace('{z}', zoom)
                        .replace('{x}', coord.x)
                        .replace('{y}', coord.y);
                },
                tileSize: new google.maps.Size(256, 256)
            });
            map.overlayMapTypes.insertAt(0, pollutionLayer);
        }
    """)
